...
namespace LMS.Repositories
{
    ...
    public class ReportRepository : IReportRepository
    {
        ...
        public ReportRepository(ILogger<ReportRepository> logger, IDbContextFactory<ApplicationDbContext> contextFactory, UserManager<User> userManager)
        {
            _logger = logger;
        ...
        public async Task<IEnumerable<ForumActivityReportDto>> GetAllForumsActivityReportAsync(User user, DateTime? startDate = null, DateTime? endDate = null, int? courseId = null)
        {
            try
            {
                ...
                var forumsQuery = context.Forums
                    .Include(f => f.Course)
                    .Include(f => f.Topics)
                        .ThenInclude(t => t.Posts)
                            .ThenInclude(p => p.Author)
        ...
        public async Task<IEnumerable<AssessmentPerformanceReportDto>> GetDifficultAssessmentsReportAsync(User user, double passRateThreshold = 70.0, DateTime? startDate = null, DateTime? endDate = null, int? courseId = null)
        {
            try
            {
                ...
                var assessmentsQuery = context.Assessments
                    .Include(a => a.Course)
                    .Include(a => a.Attempts)
                    .AsQueryable();
        ...
        public async Task<object> GetStudentEngagementReportAsync(User user, string studentId, DateTime? startDate = null, DateTime? endDate = null)
        {
            try
            {
                ...
                var studentQuery = context.Users
        ...
        public async Task<TeacherPerformanceReportDto> GetTeacherPerformanceReportAsync(User user, string teacherId, DateTime? startDate = null, DateTime? endDate = null)
        {
            try
            {
                ...
                var forumPosts = await context.ForumPosts
                    .Where(fp => fp.AuthorId == teacherId && fp.CreatedAt >= start && fp.CreatedAt <= end)
                    .CountAsync();

                var messagesReplied = await context.Messages
                    .Where(m => m.FromUserId == teacherId && m.SentAt >= start && m.SentAt <= end)
                    .CountAsync();
                ...
                return new TeacherPerformanceReportDto
                {
                    ...
                    CoursesCompleted = teacherCourses.Count(c => c.Status == CourseStatus.Archived || (c.EndDate.HasValue && c.EndDate.Value < DateTime.UtcNow))
                };
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                var reports = courses.Select(course =>
                {
                    ...
                    return new GradeDistributionReportDto
                    {
                        ...
                        HighestGrade = scores.Any() ? scores.Max() : 0,
                        LowestGrade = scores.Any() ? scores.Min() : 0,
                        MedianGrade = scores.Any() ? CalculateMedian(scores) : 0,
                        StandardDeviation = scores.Any() ? CalculateStandardDeviation(scores) : 0,
                        AGrades = scores.Count(s => s >= 90),
                        BGrades = scores.Count(s => s >= 80 && s < 90),
                        CGrades = scores.Count(s => s >= 70 && s < 80),
                        DGrades = scores.Count(s => s >= 60 && s < 70),
                        FGrades = scores.Count(s => s < 60),
                        GradeRange = scores.Any() ? $"{scores.Min()}-{scores.Max()}" : "N/A",
                        ReportGeneratedDate = DateTime.UtcNow
                    };
                }).ToList();
                ...
                var riskStudents = enrollments
                    ...
                    .Select(g => new
                    {
                        ...
                        AverageAssessmentScore = g.SelectMany(e => e.AssessmentAttempts)
                            ...
                            .DefaultIfEmpty(0)
                            .Average(),
                        LastActivity = g.Max(e => e.EnrolledAt), // Use enrollment date as last activity
        ...
        private double CalculateStandardDeviation(List<double> scores)
        {
            ...
            var sumOfSquaresOfDifferences = scores.Select(val => (val - average) * (val - average)).Sum();
            var variance = sumOfSquaresOfDifferences / scores.Count;
